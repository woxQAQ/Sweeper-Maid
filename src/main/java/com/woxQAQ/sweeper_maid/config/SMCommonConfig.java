...
public final class SMCommonConfig {
	...
	public static final ForgeConfigSpec.ConfigValue<List<? extends String>> SWEEP_BLACKLIST;
	...
	static {
		...
		EXTRA_ENTITY_TYPES = BUILDER.comment(
				...
				.defineListAllowEmpty("EXTRA_ENTITY_TYPES", List.of(
						new ResourceLocation("arrow").toString(), new ResourceLocation("spectral_arrow").toString(),
						new ResourceLocation("oceanworld", "drip_ice").toString()),
						o -> o instanceof String str && ResourceLocation.isValidResourceLocation(str));
		MESSAGE_BEFORE_SWEEP_15_30_60 = BUILDER.comment(
		...
		MESSAGE_BEFORE_SWEEP_1_10 = BUILDER.comment(
		...
		MESSAGE_AFTER_SWEEP = BUILDER.comment(
				...
				.define("MESSAGE_AFTER_SWEEP",
						"[Sweeper Maid]: $1 dropped items and $2 unnecessary entities are cleaned during this sweeping.");
		CHAT_MESSAGE_AFTER_SWEEP = BUILDER.comment(
		...
		PERMISSION_LEVEL_DUSTBIN = BUILDER.comment("Permission level of a player to open the dustbin.")
				.defineInRange("PERMISSION_LEVEL_DUSTBIN", 0, 0, 4);
		SWEEP_BLACKLIST = BUILDER.comment("Entities that will not be killed when sweeping.").defineListAllowEmpty(
				"SWEEP_BLACKLIST", List.of(),
				o -> o instanceof String str && ResourceLocation.isValidResourceLocation(str));
		BUILDER.pop();
		SPEC = BUILDER.build();
	}
	...
	public static int getItemSweepTickRemain() {
		return ITEM_SWEEP_INTERVAL.get() * SharedConstants.TICKS_PER_SECOND;
	}

	public static String getMessageAfterSweep(int droppedItems, int extraEntities) {
		return MESSAGE_AFTER_SWEEP.get().replaceAll("\\$1", String.valueOf(droppedItems)).replaceAll("\\$2",
				String.valueOf(extraEntities));
	}
